...
// Sizes
const NSInteger HLProgressViewDefaultSizeInset = 1; //px

// Animation times
const NSTimeInterval HLProgressViewStripesAnimationTime = 1.0f / 30.0f; // s

// Font name
static NSString * HLProgressViewDefaultName = @"Arial-BoldMT";

// Default progress value
const CGFloat HLProgressViewDefaultProgress = 0.3f;
...
- (UIBezierPath *)stripeWithOrigin:(CGPoint)origin bounds:(CGRect)frame orientation:(HLProgressViewStripesOrientation)orientation;
...
- (void)layoutSubviews {
  ...
  _indicatorTextLabel.font = [UIFont fontWithName:HLProgressViewDefaultName size: height];
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  if (_type == HLProgressViewTypeRounded) {
    if (_cornerRadius > 0) {
  ...
  if (_indicatorTextDisplayMode == HLProgressViewIndicatorTextDisplayModeTrack)
  {
  ...
  if (self.progress == 0 && _behavior == HLProgressViewBehaviorIndeterminate)
  {
  ...
  } else if (self.progress > 0)
  {
    ...
    {
      if (_behavior == HLProgressViewBehaviorWaiting)
      {
        if (self.progress == 1.0f)
        {
          [self drawStripes:context withRect:innerRect];
        }
      } else if (_behavior != HLProgressViewBehaviorIndeterminate)
      {
        [self drawStripes:context withRect:innerRect];
      }
    }
    ...
    // Draw the indicator text if necessary
    if (_indicatorTextDisplayMode == HLProgressViewIndicatorTextDisplayModeProgress)
    {
...
- (void)setType:(HLProgressViewType)type {
  _type = type;

  switch (type) {
    case HLProgressViewTypeRounded:
      _hideGloss = NO;
      break;
    default:
      _hideGloss = YES;
...
- (void)setProgressBarProgressTime:(NSTimeInterval)progressBarProgressTime{
    _progressBarProgressTime = progressBarProgressTime;
}
...
- (void)setBehavior:(HLProgressViewBehavior)behavior
{
  _behavior = behavior;
...
- (void)setProgress:(CGFloat)progress animated:(BOOL)animated
{
  @synchronized (self)
  {
    ...
    if (animated)
    {
      ...
      CGFloat incrementValue   = ((_progressTargetValue - _progress) * HLProgressViewStripesAnimationTime) / _progressBarProgressTime;
      self.progressTargetTimer = [NSTimer timerWithTimeInterval:HLProgressViewStripesAnimationTime
                                                         target:self
...
- (void)initializeProgressBar
{
  ...
  _type            = HLProgressViewTypeRounded;
  _progressStretch = YES;
  _hideGloss       = NO;
  _progress        = HLProgressViewDefaultProgress;
  _hideStripes     = NO;
  _hideTrack       = NO;
  _behavior        = HLProgressViewBehaviorDefault;
  _stripesColor    = [UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:0.28f];
  _cornerRadius    = 0;
    
  _progressBarProgressTime = 1;
  ...
  _indicatorTextDisplayMode = HLProgressViewIndicatorTextDisplayModeNone;
  ...
  self.stripesOrientation       = HLProgressViewStripesOrientationRight;
  self.stripesDirection         = HLProgressViewStripesDirectionRight;
  self.stripesAnimationVelocity = 1;
  self.stripesWidth             = HLProgressViewDefaultStripeWidth;
  self.stripesDelta             = HLProgressViewDefaultStripeDelta;
  self.progressBarInset         = HLProgressViewDefaultSizeInset;
...
- (UIBezierPath *)stripeWithOrigin:(CGPoint)origin bounds:(CGRect)frame orientation:(HLProgressViewStripesOrientation)orientation
{
  ...
  switch (orientation)
  {
    case HLProgressViewStripesOrientationRight:
      ...
      break;
    case HLProgressViewStripesOrientationLeft:
      [rect addLineToPoint:CGPointMake(origin.x - _stripesWidth, origin.y)];
      [rect addLineToPoint:CGPointMake(origin.x - _stripesWidth + _stripesDelta, origin.y + height)];
      [rect addLineToPoint:CGPointMake(origin.x + _stripesDelta, origin.y + height)];
      break;
...
- (void)drawTrack:(CGContextRef)context withRect:(CGRect)rect
{
  ...
  CGContextSaveGState(context);
  {
    ...
    if (_type == HLProgressViewTypeRounded && !_hideGloss)
    {
      trackHeight -= 1;
      trackWidth  -= 1;
    }
...
- (void)drawStripes:(CGContextRef)context withRect:(CGRect)rect
{
  ...
  CGContextSaveGState(context);
  {
    ...
    CGFloat yOffset = (_type == HLProgressViewTypeRounded) ? _progressBarInset : 0;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
  if ([keyPath isEqualToString:@"hideStripes"] || [keyPath isEqualToString:@"stripesAnimated"])
  {
    if (!_hideStripes && _stripesAnimated)
    {
      ...
      {
        _stripesTimer= [NSTimer timerWithTimeInterval:HLProgressViewStripesAnimationTime
                                               target:self
